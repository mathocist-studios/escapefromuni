...
public class SettingsMenu extends AbstractMenu {
    ...
    public String input() {
        ...
        if (Gdx.input.isTouched()) {
            if (!buttonCD) {
                ...
                if (textureManager.getRightArrowSprite().getBoundingRectangle()
                        .contains(new Vector2(mouse.getX(), mouse.getY())) && !alreadyClicked) {
                ...
                if (textureManager.getLeftArrowSprite().getBoundingRectangle()
                        .contains(new Vector2(mouse.getX(), mouse.getY())) && !alreadyClicked) {
                    if (difficulty.equals("Easy")) {
                        difficulty = "Hard";
                    } else if (difficulty.equals("Normal")) {
                ...
                if (textureManager.getTwoRightArrowSprite().getBoundingRectangle()
                        .contains(new Vector2(mouse.getX(), mouse.getY())) && !alreadyClicked) {
                    music = !music;
                }
                if (textureManager.getTwoLeftArrowSprite().getBoundingRectangle()
                        .contains(new Vector2(mouse.getX(), mouse.getY())) && !alreadyClicked) {
                    music = !music;
                }
                alreadyClicked = true;
        ...
        if (textureManager.getSubmitSprite().getBoundingRectangle().contains(
                new Vector2(mouse.getX(), mouse.getY()))) {
            hoveredOver = "Submit";
        } else if (textureManager.getNextButtonSprite().getBoundingRectangle().contains(
                new Vector2(mouse.getX(), mouse.getY()))) {
            hoveredOver = "Next";
        } else {
            hoveredOver = "";
    ...
    public void draw() {
        ...
        if (difficulty.equals("Easy")) {
            ...
            // textureManager.getGameMediumFont().draw(batch, "Mouse Coords: " +
            // mouse.getX() + "." +mouse.getY() + "\nSubmit Coords: " +
            // textureManager.getNextButtonSprite().getX() + "." +
            // textureManager.getNextButtonSprite().getY(),690,450);
        }
